...
var coreForm = {
    ...
    validate: function (formId) {
        ...
        $.validator.addMethod('IP4Checker', function (value, element, param) {
            return this.optional(element) || /^(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])$/.test(value);
        }, 'Invalid IP address');

        //마지막 * 입력허용
        $.validator.addMethod('IP4CheckerAll', function (value, element, param) {
            return this.optional(element) || /^(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5]|[*])$/.test(value);
        }, 'Invalid IP address');
        ...
        for (var i = 0; i < form.elements.length; i++) {
            ...
            if (!this.isEmpty(obj)) {
                ...
                var IP4Checker = obj.hasClass("cv-IP4Checker");
                var IP4CheckerAll = obj.hasClass("cv-IP4CheckerAll");
                obj.rules("add", {
                    required: required,
                    number: number,
                    email: email,
                    ...
                    IP4Checker: IP4Checker,
                    IP4CheckerAll: IP4CheckerAll,
                    messages: {
                        ...
                        IP4Checker: obj.attr("cv-IP4Checker-msg"),
                        IP4CheckerAll: obj.attr("cv-IP4CheckerAll-msg"),
    ...
    getPostData: function (formId) {
        const postData = {};
        const fieldPair = this.getSerializeForm(formId);

        // Serialize form fields as URI argument/HTTP POST data
        for (const key in fieldPair.text) {
            postData[key] = $("#" + formId).find("input[name='" + key + "']").val();
        }

        for (const key in fieldPair.password) {
            postData[key] = $("#" + formId).find("input[name='" + key + "']").val();
        }

        for (const key in fieldPair.hidden) {
            postData[key] = $("#" + formId).find("input[name='" + key + "']").val();
        }

        for (const key in fieldPair.textarea) {
            postData[key] = $("#" + formId).find("textarea[name='" + key + "']").val();
        }


        for (const key in fieldPair.radio) {
            const $selectedRadio = $("#" + formId).find("input:radio[name='" + key + "']:checked");
            postData[key] = $selectedRadio.val();
            if ($selectedRadio.data('label')) {
                postData[`${key}Nm`] = $selectedRadio.data('label');
            }
        }

        for (const key in fieldPair.checkbox) {
            postData[key] = [];
        ...
        for (const key in fieldPair.select) {
