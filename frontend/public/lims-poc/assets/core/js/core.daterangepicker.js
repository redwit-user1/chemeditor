 ...
 */
(function ($) {
    $.fn.coreDateRangePicker = function (options) {
        var coreDateRangePicker = this;
        ...
        var methods = {
            init: function () {
                ...
                if (!this.isEmpty(settings.beginInputId)) {
                    beginDatePicker = coreDateRangePicker.find('#' + settings.beginInputId).datepicker(datepickerOption);
                }
                //종료입력폼
                if (!this.isEmpty(settings.endInputId)) {
                    endDatePicker = coreDateRangePicker.find('#' + settings.endInputId).datepicker(datepickerOption);
                }
                //범위 달력 여부 확인 및 설정
                ...
                if (!methods.isEmpty(settings.clearId)) {
                    coreDateRangePicker.find('#' + settings.clearId).css('cursor', 'pointer');
                }
            },
            getBeginDate: function () {
                return coreDateRangePicker.find('#' + settings.beginInputId).val();
            },
            setBeginDate: function (date) {
                coreDateRangePicker.find('#' + settings.beginInputId).datepicker('setDate', date);
            },
            setBeginMinDate: function (date) {
                //coreDateRangePicker.find('#'+ settings.beginInputId).datepicker('setStartDate', this.addDay1(date));
                coreDateRangePicker.find('#' + settings.beginInputId).datepicker('option', 'minDate', date);
            },
            setBeginMaxDate: function (date) {
                //coreDateRangePicker.find('#'+ settings.beginInputId).datepicker('setEndDate', date);
                coreDateRangePicker.find('#' + settings.beginInputId).datepicker('option', 'maxDate', date);
            },
            getEndDate: function () {
                return coreDateRangePicker.find('#' + settings.endInputId).val();
            },
            setEndDate: function (date) {
                coreDateRangePicker.find('#' + settings.endInputId).datepicker('setDate', date);
            },
            setEndMinDate: function (date) {
                //coreDateRangePicker.find('#'+ settings.endInputId).datepicker('setStartDate', this.addDay1(date));
                coreDateRangePicker.find('#' + settings.endInputId).datepicker('option', 'minDate', date);
            },
            setEndMaxDate: function (date) {
                //coreDateRangePicker.find('#'+ settings.endInputId).datepicker('setEndDate', date);
                coreDateRangePicker.find('#' + settings.endInputId).datepicker('option', 'maxDate', date);
            },
            setToday: function (day) {
                var nowDate = new Date();
                var beginDate = new Date(nowDate.getFullYear(), nowDate.getMonth(), nowDate.getDate() - day, 0, 0, 0);
                var endDate = nowDate;
            ...
            setAfterToday: function (day) {
                var nowDate = new Date();
                var beginDate = nowDate;
                var endDate = new Date(nowDate.getFullYear(), nowDate.getMonth(), nowDate.getDate() + day, 0, 0, 0);
        ...
        var event = {
            onClear: function () {
                if (!methods.isEmpty(settings.clearId)) {
                    coreDateRangePicker.find('#' + settings.clearId).on('click', function (e) {
                        methods.clear();
                    });
                }
                return this;
            ...
            onBeginChange: function () {
                coreDateRangePicker.find('#' + settings.beginInputId).on('change', function (e, i) {
            ...
            onEndChange: function () {
                coreDateRangePicker.find('#' + settings.endInputId).on('change', function (e, i) {
