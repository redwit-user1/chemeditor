...
>
> **2026-08-07 재확인: `data/` 디렉터리는 여전히 비어 있다.** 프로토타입 배포와
> 기능 점검이 진행되는 동안에도 이 항목만은 움직이지 않았다. CLAUDE.md 가
> "전 레코드 대조" 를 타협 불가로 정해 둔 항목이므로, **회신을 확정하려면
> 5,342건 실데이터가 필요하다.** 지금 상태로는 "파이프라인은 되고 데이터는
> 못 받았다" 까지가 말할 수 있는 전부다.


- 소스: `backend/tests/fixtures/sample_foundation.sdf` (픽스처)
- 실행일: 최초 작성 시점 · 2026-08-07 재확인 시 데이터 변동 없음
- 대조 컴포넌트: **7**
- CTAB 파싱 실패: **0**
